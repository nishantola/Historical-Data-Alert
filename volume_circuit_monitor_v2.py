...
def wait_for_market():
    """Block until market opens. Prints status and sends Slack on open."""
    alerted_waiting = False
    while True:
        now = datetime.now(IST)
        t   = now.time()
        if MARKET_OPEN <= t <= MARKET_CLOSE:
            return
        if not alerted_waiting:
            msg = f"⏳ Market closed — waiting until 09:15 IST (now {t.strftime('%H:%M')} IST)"
            print(f"\n  {msg}")
            send_slack_alert(msg)
            alerted_waiting = True
...
def run_monitor():
    ...
    while True:
        ...
        # ── Stop scanning if market has closed ────────────────────
        if not is_market_open():
            send_slack_alert("🔴 Market closed (15:30 IST) — pausing until tomorrow 09:15")
            print("  [Market closed — pausing]\n")
