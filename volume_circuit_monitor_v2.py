...
# ================= SLACK CONFIG =================
SLACK_WEBHOOK_CIRCUIT = os.getenv("SLACK_WEBHOOK_CIRCUIT")
 
def send_slack_alert(message):
    payload = {"text": message}
    try:
        response = requests.post(SLACK_WEBHOOK_CIRCUIT, json=payload, timeout=5)
...
MARKET_START = dtime(9, 10)   # script wakes up, authenticates, loads symbols
MARKET_CLOSE = dtime(15, 30)  # script exits
 
def is_market_open():
    t = datetime.now(IST).time()
    return MARKET_START <= t <= MARKET_CLOSE
 
def wait_for_market():
    """
    Logic:
      - After midnight (00:00–09:10 IST) → wait until 09:10, then start
      - 09:10–15:30                       → run normally
      - After 15:30                        → exit immediately
    """
    t = datetime.now(IST).time()
 
    # Already past close — exit
    if t > MARKET_CLOSE:
        msg = f"🔴 Market closed ({t.strftime('%H:%M')} IST) — stopping script"
        print(f"\n  {msg}")
        send_slack_alert(msg)
        raise SystemExit(0)
 
    # Before 09:10 (post-midnight window) — wait
    if t < MARKET_START:
        msg = (f"⏳ Waiting for market start — will begin at 09:10 IST "
               f"(now {t.strftime('%H:%M')} IST)")
        print(f"\n  {msg}")
        send_slack_alert(msg)
        while True:
            t = datetime.now(IST).time()
            if t >= MARKET_START:
                print("  [09:10 IST — starting now]")
                return
            time.sleep(30)   # check every 30s near open
...
def run_monitor():
    ...
    while True:
        # ── Wait for market hours (09:15–15:30 IST) ──────────────
        if not is_market_open():
            wait_for_market()
            send_slack_alert("🟢 Market open — resuming circuit scan")
            print("  [Market open — resuming scan]\n")
        ...
        # ── Stop script if market has closed ─────────────────────
        if not is_market_open():
            msg = "🔴 Market closed (15:30 IST) — stopping script"
            print(f"\n  {msg}")
            send_slack_alert(msg)
            raise SystemExit(0)
